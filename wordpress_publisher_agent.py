...
def get_wordpress_credentials():
    ...
    return {"url": wp_url, "user": wp_user, "password": wp_app_password}
...
def create_wordpress_post(
    wp_credentials, content_piece, media_id=None, keywords=None, preview=False
):
    """
    ...
    try:
        ...
        content_md = content_piece.get("draft_text", "")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="WordPress Publisher Agent - Publish content to WordPress"
    )
    parser.add_argument("--content-id", help="ID of the content piece to process")
    parser.add_argument(
        "--preview", action="store_true", help="Preview the post without publishing"
    )
        ...
        if image_data:
            ...
            if image_path:
                ...
                media_id = upload_image_to_wordpress(
                    wp_credentials, image_path, image_title
                )

    # Create WordPress post
    post_id, post_url = create_wordpress_post(
        wp_credentials, content_piece, media_id, keywords, args.preview
    )
