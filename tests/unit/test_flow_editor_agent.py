...
spec = importlib.util.spec_from_file_location(
    "flow_editor_agent",
    os.path.join(REPO_ROOT, "agents", "flow-editor-agent", "index.py"),
)
flow_editor_agent = importlib.util.module_from_spec(spec)
spec.loader.exec_module(flow_editor_agent)
sys.modules["flow_editor_agent"] = flow_editor_agent

# Import functions to test
from flow_editor_agent import (
    setup_openai,
    get_supabase_client,
    get_content_piece,
    ...
    generate_mock_improved_flow,
    save_flow_edited_to_database,
    save_flow_edited_to_file
)
...
"""
    ...
    def test_get_supabase_client(self, mock_getenv):
        ...
        with patch("flow_editor_agent.create_client") as mock_create_client:
    ...
    def test_get_content_piece_no_id(self):
        ...
        mock_supabase.table.assert_called_once_with("content_pieces")
        mock_supabase.table.return_value.select.assert_called_once_with("*")
        mock_supabase.table.return_value.select.return_value.eq.assert_called_once_with("status", "written")
    ...
    def test_improve_flow_with_ai(self, mock_print):
        ...
        mock_response.choices = [MagicMock(message=MagicMock(content=self.mock_improved_text))]
    ...
    def test_save_flow_edited_to_database(self, mock_print):
        ...
        result = save_flow_edited_to_database(mock_supabase, "test-content-id", self.mock_improved_text)
    ...
    def test_save_flow_edited_to_file(self, mock_file_open):
        ...
        filename = save_flow_edited_to_file(content_id, content_title, self.mock_improved_text)
