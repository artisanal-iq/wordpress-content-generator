...
def full_pipeline(args):
    ...
    print(f"{BOLD}Step 5: Running Line Editor Agent for {len(content_pieces)} content pieces{ENDC}")

    line_success_count = 0
    for i, content_id in enumerate(content_pieces):
        print(f"{BLUE}Processing content piece {i+1} of {len(content_pieces)} with Line Editor Agent{ENDC}")
